...
class AgentClass(BaseElementClass):
  def __init__(self,name,ifPkg,clk,rst,activity,sub_env_path):
...
class SubEnvironmentClass(BaseElementClass):
  def __init__(self,name,envPkg,numAgents,agent_index):
...
class EnvironmentClass(BaseGeneratorClass):
  ...
  def addAgent(self,name,ifPkg,clk,rst,activity='ACTIVE',sub_env_path='environment'):
    """Add an agent instantiation to the definition of this environment class"""
    self.agents.append(AgentClass(name,ifPkg,clk,rst,activity,sub_env_path))
    self.agentIndex = self.agentIndex + 1
    if (ifPkg not in self.agent_packages):
      self.agent_packages.append(ifPkg)

  def addSubEnv(self,name,envPkg,numAgents):
    """Add a sub environment instantiation to the definition of this environment class"""
    self.subEnvironments.append(SubEnvironmentClass(name,envPkg,numAgents,self.agentIndex))
  ...
  def addAnalysisComponent(self, name, pType):
    ...
    self.addAnalysisComponentType(pType)
...
class BenchClass(EnvironmentClass):
  ...
  def addBfm(self,name,ifPkg,clk,rst,activity,sub_env_path='environment'):
